...
const getDueDateStatus = (dueDate: string, status: TaskStatus) => {
  ...
  return { text: due.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), class: 'text-gray-500 dark:text-slate-400' };
};
...
const Onboarding: React.FC<OnboardingProps> = ({ user }) => {
  ...
  const filteredTasks = useMemo(() => {
    let result = filterCategory === 'all' ? visibleTasks : visibleTasks.filter(t => t.category === filterCategory);
  ...
  const handleToggleTask = (taskId: string) => {
    setTasks(tasks.map(task => {
      if (task.id === taskId) {
        ...
        createAuditLog(user, newStatus === TaskStatus.DONE ? 'COMPLETE_TASK' : 'UNCOMPLETE_TASK', `Task: "${task.title}"`);
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-amber-600 dark:text-amber-300">
              You have {unacknowledgedPolicies} required {unacknowledgedPolicies === 1 ? 'policy' : 'policies'} to review.
            </p>
          </div>
        </div>
      )}
      ...
      <Card>
        <div className="flex flex-col md:flex-row md:items-center gap-6">
          ...
          <div className="flex items-center justify-center w-24 h-24 rounded-full bg-gradient-to-br from-gray-100 to-gray-200 dark:from-slate-700 dark:to-slate-800">
            <div className={`flex items-center justify-center w-20 h-20 rounded-full bg-gradient-to-br ${getProgressColor()}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
              ...
              <div className="flex items-center gap-3">
                <select
                  ...
                  className="text-sm bg-gray-50 dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500"
                >
                  {categories.map(cat => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <h4 className={`font-medium ${task.status === TaskStatus.DONE ? 'line-through text-gray-500' : 'text-gray-900 dark:text-white'}`}>
                          {task.title}
                        </h4>
                      ...
                      <div className="flex items-center gap-4 mt-1 text-sm">
                        {dueStatus && (
                          <span className={dueStatus.class}>{dueStatus.text}</span>
                        )}
                        <span className="text-gray-400 dark:text-slate-500">~{task.estimatedTime}h</span>
                      </div>
                    </div>
        ...
        <div>
          <Card>
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Policy Acknowledgment</h3>
              ...
              {policies.map(policy => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-medium text-gray-900 dark:text-white text-sm">{policy.title}</h4>
      ...
      <Modal isOpen={isPolicyModalOpen} onClose={() => setIsPolicyModalOpen(false)} title={selectedPolicy?.title || ''}>
        <div className="space-y-6">
          <div className="max-h-60 overflow-y-auto p-4 bg-gray-50 dark:bg-slate-700/50 rounded-xl text-sm text-gray-600 dark:text-slate-300">
            <p>This document outlines the procedures and expectations regarding this policy. All {config.terminology.employees.toLowerCase()} are required to read and understand this policy thoroughly.</p>
          ...
          <label className="flex items-start gap-3 p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl cursor-pointer">
            <input
              type="checkbox"
