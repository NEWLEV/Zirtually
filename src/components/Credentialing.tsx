...
const MOCK_APPLICATIONS: CredentialApplication[] = [
  ...
  },
  {
    ...
    requiredDocuments: [
      ...
      {
        name: 'Practice Address Verification',
        status: 'rejected',
        notes: 'Document expired, please upload current version',
      },
...
const CREDENTIAL_REQUIREMENTS = [
  {
    ...
    items: ['State Medical License', 'DEA Registration', 'Controlled Substance License'],
  },
  {
    category: 'Board Certifications',
    items: ['Primary Specialty', 'Subspecialty (if applicable)'],
  },
...
export const Credentialing: React.FC<CredentialingProps> = ({ user, setActiveView }) => {
  ...
  const [selectedApplication, setSelectedApplication] = useState<CredentialApplication | null>(
    null
  );
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'draft':
        return 'bg-bg-secondary text-text-secondary dark:text-dark-text-secondary border border-border-light';
      default:
  ...
  return (
    <div className="space-y-6">
      <Breadcrumbs
        items={[{ label: 'Work' }, { label: 'Credentialing' }]}
        setActiveView={setActiveView}
      />
      {/* Header */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-action-primary to-action-secondary p-8 text-text-inverse">
        ...
        <div className="relative z-10">
          ...
          <p className="text-white/90 mb-6">
            Manage your licenses, certifications, and credentialing applications
          </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 text-center border border-white/10">
              ...
              <div className="text-sm text-white/80 font-medium">Active Credentials</div>
            </div>
            <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 text-center border border-white/10">
              <div className="text-3xl font-bold text-amber-300">{expiringCredentials}</div>
              <div className="text-sm text-white/80 font-medium">Expiring Soon</div>
            </div>
            <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 text-center border border-white/10">
              <div className="text-3xl font-bold text-red-300">{expiredCredentials}</div>
              <div className="text-sm text-white/80 font-medium">Expired</div>
            </div>
            <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 text-center border border-white/10">
              <div className="text-3xl font-bold">{pendingApplications}</div>
              <div className="text-sm text-white/80 font-medium">Applications</div>
            </div>
          </div>
        </div>
      ...
      {(expiringCredentials > 0 || expiredCredentials > 0) && (
        <Card
          variant="bordered"
          className="p-4 border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20"
        >
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-amber-700 dark:text-amber-300 mt-1">
                You have {expiringCredentials} credential(s) expiring soon and {expiredCredentials}{' '}
                expired credential(s). Please initiate renewal processes to maintain compliance.
              </p>
            </div>
      ...
      <div className="flex gap-2 border-b border-gray-200 dark:border-gray-700 overflow-x-auto">
        {[
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-3 border-b-2 transition-colors whitespace-nowrap font-medium ${
              activeTab === tab.id
            ...
            <Card variant="glass" className="p-6">
              ...
              <div className="space-y-3">
                {MOCK_CREDENTIALS.filter(
                  c => (c.status === 'expiring_soon' || c.status === 'expired') && c.expiryDate
                )
                  .sort(
                  ...
                  .map(credential => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <Button
                          size="sm"
                          variant={credential.status === 'expired' ? 'primary' : 'secondary'}
                        >
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-text-tertiary dark:text-dark-text-secondary">
                      Overall Compliance
                    </span>
                    <span className="font-medium text-text-primary dark:text-dark-text">85%</span>
                  </div>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                  filterStatus === status
                    ...
                    : 'bg-bg-secondary text-text-secondary dark:text-dark-text-secondary border border-border-light'
                }`}
              >
                {status === 'all'
                  ? 'All'
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center ${
                        credential.type === 'license' ? 'bg-status-info/10' : 'bg-action-primary/10'
                      }`}
                    >
                      <Icon
                        ...
                        size={20}
                        className={
                          credential.type === 'license' ? 'text-status-info' : 'text-action-primary'
                        }
                      />
                    </div>
                    <div>
                  ...
                  <span
                    className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(credential.status)}`}
                  >
                    <Icon
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    <p className="text-text-tertiary dark:text-dark-text-secondary">Expiry Date</p>
                    <p
                ...
                <div className="flex items-center justify-between text-sm">
                  <div className="flex items-center gap-4">
                    <span className="text-gray-500 dark:text-gray-400">
                      ...
                      {application.requiredDocuments.filter(d => d.status === 'uploaded').length}/
                      {application.requiredDocuments.length} documents
                    </span>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Icon
                          ...
                          size={20}
                          className={hasCredential ? 'text-status-success' : 'text-text-tertiary'}
                        />
                        <span className="text-text-primary dark:text-dark-text">{item}</span>
                      </div>
                      {!hasCredential && (
        ...
        >
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <span
                className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(selectedCredential.status)}`}
              >
              ...
              {selectedCredential.verificationUrl && (
                <div>
                  ...
                  <a
                    ...
                    className="text-action-primary hover:underline text-sm font-medium"
                  >
                    Verify Online →
                  </a>
                </div>
        ...
        >
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <span
                className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(selectedApplication.status)}`}
              >
            ...
            <ProgressBar
              value={selectedApplication.completionPercentage}
              max={100}
              color="emerald"
            />
      ...
      <Modal
        ...
        title="Credentials Requiring Attention"
      >
        <div className="space-y-4">
          {MOCK_CREDENTIALS.filter(c => c.status === 'expiring_soon' || c.status === 'expired').map(
            credential => (
              <div
                key={credential.id}
              ...
              >
                <div>
                  ...
                  <p
                    className={`text-sm ${credential.status === 'expired' ? 'text-red-600' : 'text-amber-600'}`}
                  >
                    {credential.status === 'expired'
                      ? `Expired ${formatDate(credential.expiryDate)}`
