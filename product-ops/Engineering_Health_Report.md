...
- [ ] **Implement Deployment Targets**: `production.yml` has clean logic but placeholder implementation.
    -   *Action:* Configure Vercel/Netlify/AWS S3 deployment steps.
...
- [ ] **Release Strategy**: Current workflow supports `release/*` branches.
    -   *Action:* Formalize semantic versioning and tagging triggers.

### Developer Workflow
- [ ] **Pre-commit Hooks**: No Husky configuration found.
    -   *Action:* Install Husky + lint-staged to prevent bad code from being committed.
- [ ] **Documentation**: `README.md` is minimal.
    -   *Action:* Update README with new `src/` structure and "How to Run Tests" guide.
